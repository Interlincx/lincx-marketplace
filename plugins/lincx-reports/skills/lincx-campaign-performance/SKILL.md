...
2. **Resolve the campaign(s):**
   - Page through `list_campaigns({ network_id, limit: 100, offset })` and match by case-insensitive substring on `name`.
...
4. **Run the report** — one campaign:
   ...
   `report_query({ network_id, dimensionSetId, startDate, endDate, groupBy: ["date"], filter: { campaign: "<name>" } })`
