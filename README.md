...
| **`lincx-analysis`** | Zone analysis. Tier analysis: the platform's deterministic engine computes the tiers, ranks and risk flags; Claude writes the rationale, justifications and next actions on top. Exposure diagnostics: reads the exposure join and the scoring probe and writes the crowd-out / explore-exploit read, with every inferred sentence labelled. The analysis prompt is a markdown file you can edit instead of a deploy. |
